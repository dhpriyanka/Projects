-- 1)My partner and I want to come by each of stores in person and meet the manages. 
-- Please send manger’s names at each store with the full address of each property street address, district, city and country please
...
-- 8)We are interested how well you have covered the most-awarded actors.Of all the actors with three types of awards, for what % of them do we carry a film?
-- And how about for actors with two types of awards? Same question. Finally how about actors with just one award

select 
case when aa.awards= "Emmy, Oscar,Tony" then "3 awards"
when aa.awards in ("Emmy, Oscar", "Emmy, Tony","Emmy, Tony") then "2 awards"
else "1 award" end as Number_of_Awards,
avg(case when aa.actor_id is null then 0 else 1 end) as pct_w_1_film 
from actor_award aa
